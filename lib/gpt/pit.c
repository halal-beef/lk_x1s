...
struct pit_entry *pit_get_virtual_partition_entry(int partition)
{
	...
	print_lcd_update(FONT_BLACK, FONT_WHITE, "0: S: %x N: %x", new->blkstart, new->blknum);
	...
	if(partition == 1)
	{
		...
		print_lcd_update(FONT_BLACK, FONT_WHITE, "1: S: %x N: %x", new->blkstart, new->blknum);
	}
	else if(partition == 2)
	{
		...
		print_lcd_update(FONT_BLACK, FONT_WHITE, "2: S: %x N: %x", new->blkstart, new->blknum);
	}
	else
	{
...
struct pit_entry *pit_get_part_info(const char *name)
{
	...
	for (u32 i = 0; i < pit.count; i++) {
		...
		{
					print_lcd_update(FONT_BLACK, FONT_WHITE, "N: S: %x N: %x", ptn->blkstart, ptn->blknum);
			return ptn;
		}
	}
