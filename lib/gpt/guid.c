...
int uid_str_to_bin(char *uid_str, unsigned char *uid_bin, int str_format)
{
	...
	if (str_format == UID_STR_STD) {
		tmp32 = cpu_to_be32(strtoul(uid_str, NULL, 16));
		memcpy(uid_bin, &tmp32, 4);

		tmp16 = cpu_to_be16(strtoul(uid_str + 9, NULL, 16));
		memcpy(uid_bin + 4, &tmp16, 2);

		tmp16 = cpu_to_be16(strtoul(uid_str + 14, NULL, 16));
		memcpy(uid_bin + 6, &tmp16, 2);
	} else {
		tmp32 = strtoul(uid_str, NULL, 16);
		memcpy(uid_bin, &tmp32, 4);

		tmp16 = strtoul(uid_str + 9, NULL, 16);
		memcpy(uid_bin + 4, &tmp16, 2);

		tmp16 = strtoul(uid_str + 14, NULL, 16);
		memcpy(uid_bin + 6, &tmp16, 2);
	}

	tmp16 = cpu_to_be16(strtoul(uid_str + 19, NULL, 16));
	memcpy(uid_bin + 8, &tmp16, 2);

	tmp64 = cpu_to_be64(strtoull(uid_str + 24, NULL, 16));
