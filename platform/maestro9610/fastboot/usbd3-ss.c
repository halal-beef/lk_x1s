...
/* from /arch/arm/cpu/armv8/exynos/clock.c in u-boot*/
...
/* USB PHY reference clock frequency */
...
u64 g_uCntOfBulkOutTrb, g_uCntOfBulkInTrb;
...
extern void EXYNOS_USBD_CLEAR_IRQ(void)
{
	do {
		;
	} while (0);
...
#include "usbdrd-exynos-phy.c"

#if defined(CONFIG_CPU_EXYNOS7885)
...
int pll_enable(void *pllcon_base)
{
	int cnt;
	u32 pll_pms, reg;

	pll_pms = (400 << MDIV_BIT16) | (13 << PDIV_BIT8) | (4 << SDIV_BIT0);

	if((readl(pllcon_base + PHY_EXYNOS_USB_PLL_CON0) & PLL_ENABLE_BIT) == 0)
		writel(pll_pms, pllcon_base + PHY_EXYNOS_USB_PLL_CON0);
	else
		writel(PLL_ENABLE_BIT | pll_pms, pllcon_base + PHY_EXYNOS_USB_PLL_CON0);

	reg = readl(pllcon_base + PHY_EXYNOS_USB_PLL_CON0);

	if((reg & PLL_ENABLE_BIT) == 0)
		writel(reg | PLL_ENABLE_BIT, pllcon_base + PHY_EXYNOS_USB_PLL_CON0);

	for (cnt = 1000; cnt != 0; cnt--) {
		reg = readl(pllcon_base + PHY_EXYNOS_USB_PLL_CON0);

		if (reg & PLL_LOCKED_BIT)
			break;
	...
	reg = readl(pllcon_base + PHY_EXYNOS_USB_PLL_CON0);
	reg |= PLL_MUX_SEL_BIT;
	writel(reg, pllcon_base + PHY_EXYNOS_USB_PLL_CON0);

	for (cnt = 1000; cnt != 0; cnt--) {
		reg = readl(pllcon_base + PHY_EXYNOS_USB_PLL_CON0);

		if (!(reg & PLL_MUX_BUSY_BIT))
			break;
...
static void exynos_usb_set_descriptor_tlb(void)
{
	...
	oUsbDev3.m_oDesc.oDescInterface.bLength=INTERFACE_DESC_SIZE;
	...
	oUsbDev3.m_oDesc.oDescInterface.bInterfaceClass=0xff;
...
static void exynos_usb_softreset_core(void)
{
	...
	u_delay(10);	// s/w must wait at least 3 phy clocks(1/60Mz * 3 = 48ns) before accessing the phy domain
}
...
static void exynos_usb_fill_trb(usbdev3_trb_ptr_t pTrb, u64 uBufAddr, u32 uLen, u32 uTrbCtrl, u32 uHwo)
{
	...
	pTrb->buf_ptr_h = virt_to_phys((void *)(uBufAddr >> 32));
...
static int exynos_usb_start_ep0_setup_rx(void)
{
	...
	exynos_usb_fill_trb(oUsbDev3.m_oSetupTrbPtr, (u64)&oUsbDev3.m_oDeviceRequest,
		oUsbDev3.m_uControlEPMaxPktSize, usbdev3_trb_ctrl.data, 1);

	// . Issue Start Xfer for EP0-OUT
	//----------------------------
	if (!exynos_usb_start_ep_xfer(USBDEV3_EP_DIR_OUT, 0, (u64)oUsbDev3.m_oSetupTrbPtr, 0, &oUsbDev3.m_uTriOut[0]))
	{
		return 0;
	}
...
static int exynos_usb_init_core(USBDEV3_SPEED_e eSpeed)
{
	...
	/*
	 * Bus configuration
	 * should be set to Cacheable/Modifiable
	 * refer to Synopsys databook 6.2.1.1 Global SoC Bus Configuration Registers 0
	 */
...
static int exynos_usb_start_ep0_in_xfer(u32 pBufAddr, u32 uLen)
{
	...
	//----------------------------
	if (!exynos_usb_start_ep_xfer(USBDEV3_EP_DIR_IN, 0, (u64)oUsbDev3.m_oInTrbPtr, 0, &oUsbDev3.m_uTriIn[0]))
	{
		return 0;
	}
...
static int exynos_usb_start_ep0_out_xfer(u32 pBufAddr, u32 uLen)
{
	...
	// . Issue Start Xfer for EP0-OUT
	//----------------------------
	if (!exynos_usb_start_ep_xfer(USBDEV3_EP_DIR_OUT, 0, (u64)oUsbDev3.m_oOutTrbPtr, 0, &oUsbDev3.m_uTriOut[0]))
	{
		return 0;
	}
...
static void exynos_usb_setup_out_status_phase(void)
{
	...
	exynos_usb_start_ep0_out_xfer((u64)&oUsbDev3.m_oDeviceRequest, 0);
}
...
static int exynos_transfer_packetsize(void)
{
	...
	if (oUsbDev3.m_eSpeed == USBDEV3_SPEED_SUPER)
		pkt_size = SUPER_SPEED_BULK_PKT_SIZE;
	else if (oUsbDev3.m_eSpeed == USBDEV3_SPEED_HIGH)
		pkt_size = HIGH_SPEED_BULK_PKT_SIZE;
	else
		pkt_size = FULL_SPEED_BULK_PKT_SIZE;
...
int exynos_move_trb_buff(int transfer_size, int remain)
{
	g_ucTempDownBuf = g_ucTempDownBuf + transfer_size;

	if (remain < (HOST_TRANSFER_SIZE * 2))
		oUsbDev3.m_uBulkEPMaxPktSize = exynos_transfer_packetsize();

	return 0;
}

int exynos_init_trb_buf(void)
{
	g_ucTempDownBuf = (u8 *)exynos_usb_malloc(SZ_4K + 1, USBDEV3_MDWIDTH/8);
	oUsbDev3.m_uBulkEPMaxPktSize = exynos_transfer_packetsize();
...
void exynos_usb_handle_setup(void)
{
	...
	switch(oUsbDev3.m_oDeviceRequest.bRequest) {
		...
		case STANDARD_GET_CONFIGURATION:
			exynos_usb_start_ep0_in_xfer((u64)&g_usConfig, 1);
			break;
		...
		case STANDARD_GET_DESCRIPTOR:
			switch (oUsbDev3.m_oDeviceRequest.wValue_H)
			{
				case DEVICE_DESCRIPTOR:
					...
					{
						exynos_usb_start_ep0_in_xfer(((u64)&(oUsbDev3.m_oDesc.oDescDevice))+0, oUsbDev3.m_uDeviceRequestLength);
					}
					else
					{
						exynos_usb_start_ep0_in_xfer(((u64)&(oUsbDev3.m_oDesc.oDescDevice))+0, DEVICE_DESC_SIZE);
					}
					break;
				...
				case CONFIGURATION_DESCRIPTOR:
					...
					// oUsbDev3.m_uEpZeroTransferLength = CONFIG_DESC_TOTAL_SIZE;
						...
						{
							exynos_usb_start_ep0_in_xfer(((u64)&(oUsbDev3.m_oDesc.oDescConfig))+0, oUsbDev3.m_uDeviceRequestLength);
						}
						else
						{
							exynos_usb_start_ep0_in_xfer(((u64)&(oUsbDev3.m_oDesc.oDescConfig))+0, oUsbDev3.m_oDesc.oDescConfig.wTotalLengthL);
						}
					}
					else		// for win2k
					{
						...
						{
							exynos_usb_start_ep0_in_xfer(((u64)&(oUsbDev3.m_oDesc.oDescConfig))+0, CONFIG_DESC_SIZE);
						}
					}
					break;
				...
				case STRING_DESCRIPTOR :
					...
					{
						case 0:
							exynos_usb_start_ep0_in_xfer((u64)string_desc0, STRING_DESC0_SIZE);
							break;
						case 1:
							exynos_usb_start_ep0_in_xfer((u64)string_desc1, string_desc1_size);
							break;
						case 2:
							exynos_usb_start_ep0_in_xfer((u64)string_desc2, string_desc2_size);
							break;
						case 3:
							exynos_usb_start_ep0_in_xfer((u64)string_desc3, string_desc3_size);
							break;
						default:
							break;
					}
				...
				case ENDPOINT_DESCRIPTOR:
					...
					switch(oUsbDev3.m_oDeviceRequest.wValue_L&0xf)
					{
						case 0:
							if (oUsbDev3.m_eSpeed == USBDEV3_SPEED_SUPER)
								exynos_usb_start_ep0_in_xfer((u64)&(oUsbDev3.m_oSSDesc.oDescEp0), ENDPOINT_DESC_SIZE+ENDPOINT_COMP_DESC_SIZE);
							else
								exynos_usb_start_ep0_in_xfer((u64)&(oUsbDev3.m_oDesc.oDescEp0), ENDPOINT_DESC_SIZE);
							break;
						case 1:
							if (oUsbDev3.m_eSpeed == USBDEV3_SPEED_SUPER)
								exynos_usb_start_ep0_in_xfer((u64)&(oUsbDev3.m_oSSDesc.oDescEp1), ENDPOINT_DESC_SIZE+ENDPOINT_COMP_DESC_SIZE);
							else
								exynos_usb_start_ep0_in_xfer((u64)&(oUsbDev3.m_oDesc.oDescEp1), ENDPOINT_DESC_SIZE);
							break;
						default:
							break;
					}
				...
				case DEVICE_QUALIFIER:	// only supported in over 2.0
					...
					{
						exynos_usb_start_ep0_in_xfer((u64)qualifier_desc, oUsbDev3.m_uDeviceRequestLength);
					}
					else
					{
						exynos_usb_start_ep0_in_xfer((u64)qualifier_desc, 10);
					}
					break;
				...
				case OTHER_SPEED_CONFIGURATION :
					...
					if (oUsbDev3.m_eSpeed == USBDEV3_SPEED_HIGH)
					{
						...
						{
							exynos_usb_start_ep0_in_xfer((u64)config_high, 9);
						}
						else if(oUsbDev3.m_uDeviceRequestLength ==32)
						{
							exynos_usb_start_ep0_in_xfer((u64)config_high_total, 32);
						}
					}
					else if (oUsbDev3.m_eSpeed == USBDEV3_SPEED_FULL)
					{
						if (oUsbDev3.m_uDeviceRequestLength ==9)
						{
							exynos_usb_start_ep0_in_xfer((u64)config_full, 9);
						}
						else if(oUsbDev3.m_uDeviceRequestLength ==32)
						{
							exynos_usb_start_ep0_in_xfer((u64)config_full_total, 32);
						}
					}
					else	// super
					{
				...
				case BOS :
					if (oUsbDev3.m_uDeviceRequestLength == BOS_DESC_SIZE)
						exynos_usb_start_ep0_in_xfer((u64)&oUsbDev3.m_oSSDesc.oDescBos, BOS_DESC_SIZE);
					else
						exynos_usb_start_ep0_in_xfer((u64)&oUsbDev3.m_oSSDesc.oDescBos, BOS_DESC_TOTAL_SIZE);
					break;
			}
			break;
		...
		case STANDARD_GET_STATUS:
			switch(oUsbDev3.m_oDeviceRequest.bmRequestType)
			{
				case  (0x80):	//device
					...
					exynos_usb_start_ep0_in_xfer((u64)&oStatusGet.Device, oUsbDev3.m_oDeviceRequest.wLength_L);
					break;

				case  (0x81):	//interface
					oStatusGet.Interface=0;
					exynos_usb_start_ep0_in_xfer((u64)&oStatusGet.Interface, 1);
					break;

				case  (0x82):	//endpoint
					if ((oUsbDev3.m_oDeviceRequest.wIndex_L & 0x7f) == CONTROL_EP)
						exynos_usb_start_ep0_in_xfer((u64)&oStatusGet.EndpointCtrl, 1);

					if ((oUsbDev3.m_oDeviceRequest.wIndex_L & 0x7f) == BULK_IN_EP)
						exynos_usb_start_ep0_in_xfer((u64)&oStatusGet.EndpointIn, 1);

					if ((oUsbDev3.m_oDeviceRequest.wIndex_L & 0x7f) == BULK_OUT_EP)
						exynos_usb_start_ep0_in_xfer((u64)&oStatusGet.EndpointOut, 1);
					break;
		...
		case STANDARD_GET_INTERFACE:
			exynos_usb_start_ep0_in_xfer((u64)&oInterfaceGet.AlternateSetting, 1);
			break;
...
static void exynos_usb_handle_ep_out_xfer_complete(void)
{
	...
	if (g_uCntOfDescOutComplete == 0)
	{
		...
		exynos_usb_free((u64)g_pBulkOutTrb0);
		...
		if (usRxCnt == 10)		//Upload Request
		{
			...
			if (usCheck == 0x1)
			{
				...
				if (oUsbDev3.m_uUploadSize>0)
				{
					...
					{
						exynos_usb_fill_trb(pBulkInTrb, (u64)(oUsbDev3.m_pUpPt+usCapTrbBufSiz*i), usCapTrbBufSiz, usbdev3_trb_ctrl.data, 1);
						pBulkInTrb++;
					}
					...
					exynos_usb_fill_trb(pBulkInTrb, (u64)(oUsbDev3.m_pUpPt+usCapTrbBufSiz*i), uLastBufSize, usbdev3_trb_ctrl.data, 1);
					//
					////

					// . Issue Start Xfer for Bulk In Xfer
					//----------------------------
					if (!exynos_usb_start_ep_xfer(USBDEV3_EP_DIR_IN, BULK_IN_EP, (u64)g_pBulkInTrbArray_Base, 0, &oUsbDev3.m_uTriIn[BULK_IN_EP]))
					{
						return;
					}
				}
		...
		else		//Download Request
		{
			...
			if (oUsbDev3.m_uDownloadFileSize>usRxCnt)	//there are more data to be received
			{
				...
				{
					exynos_usb_fill_trb(pBulkOutTrb, (u64)(oUsbDev3.m_pDownPt+usCapTrbBufSiz*i), usCapTrbBufSiz, usbdev3_trb_ctrl.data, 1);
					pBulkOutTrb++;
				}
				...
				exynos_usb_fill_trb(pBulkOutTrb, (u64)(oUsbDev3.m_pDownPt+usCapTrbBufSiz*i), uLastBufSize, usbdev3_trb_ctrl.data, 1);
				//
				////

				// . Issue Start Xfer for Bulk Out Xfer
				//----------------------------
				if (!exynos_usb_start_ep_xfer(USBDEV3_EP_DIR_OUT, BULK_OUT_EP, (u64)g_pBulkOutTrbArray_Base, 0, &oUsbDev3.m_uTriOut[BULK_OUT_EP]))
				{
					return;
				}
			}
	...
	else
	{
		...
		exynos_usb_free((u64)g_pBulkOutTrbArray_Base);
