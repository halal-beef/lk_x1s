...
#define USB_LINK_BASE			0x10C00000
#define USB_PHY_BASE			0x10B00000
...
{
	chip_id[0] = readl(EXYNOS9820_PRO_ID + CHIPID0_OFFSET);
	chip_id[1] = readl(EXYNOS9820_PRO_ID + CHIPID1_OFFSET) & 0xFFFF;
}
...
void sec_set_reboot_magic(int magic, int offset, int mask)
{
	...
	tmp = readl(EXYNOS9820_POWER_BASE + SEC_DEBUG_MAGIC_INFORM);

	mask <<= offset;
	tmp &= ~mask;
	tmp |= magic << offset;

	writel(tmp, EXYNOS9820_POWER_BASE + SEC_DEBUG_MAGIC_INFORM);
}
...
void platform_do_reboot(const char *cmd_buf)
{
	...
	if (!memcmp(cmd_buf, "reboot-bootloader", strlen("reboot-bootloader"))) {
		writel(REBOOT_MODE_LK3RD, EXYNOS9820_POWER_SYSIP_DAT0);
		...
		reason = SEC_RESET_REASON_RECOVERY;
		writel(REBOOT_MODE_FASTBOOT_USER, EXYNOS9820_POWER_SYSIP_DAT0);
	} else if(!memcmp(cmd_buf, "reboot-download", strlen("reboot-download"))) {
		reason = SEC_RESET_REASON_DOWNLOAD;
	} else {
		writel(0, EXYNOS9820_POWER_SYSIP_DAT0);
	}

	/* Set reboot reason */
	writel(reason, EXYNOS9820_POWER_BASE + SEC_DEBUG_PANIC_INFORM);

	writel(0, CONFIG_RAMDUMP_SCRATCH);
	sec_set_reboot_magic(SEC_REBOOT_NORMAL, SEC_REBOOT_END_OFFSET, 0xFF);
	writel(0, EXYNOS9820_POWER_RST_STAT);
	writel(readl(EXYNOS9820_POWER_SYSTEM_CONFIGURATION) | 0x2, EXYNOS9820_POWER_SYSTEM_CONFIGURATION);
